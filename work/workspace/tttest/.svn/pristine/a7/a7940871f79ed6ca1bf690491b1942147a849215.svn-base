/*
 * @(#)ReflectionUtils.java     V1.0.0      @2014-6-18
 *
 * Project:unpcommon
 *
 * Modify Information:
 ...
 */
package com.unicompayment.fip.common.utils.principle;
...
import com.unicompayment.fip.common.utils.date.SimpleDateFormatFactory;
...
public abstract class ReflectionUtils {
	...
	public static void copyProperties(Object fromobj, Object toobj) {
		...
		for (Method method : methods) {
			...
			if(methodName.length()>3 && methodName.substring(0,3).equals("get")){
				...
				try {
					setMethod = toobj.getClass().getMethod(setMethodName,new Class<?>[]{cl});
				...
				if (setMethod != null){
						invokeMethod(toobj,
								setMethodName,setMethod.getParameterTypes(),
								new Object[] { invokeMethod(fromobj,
	...
	private static Object handleValueType(Object obj, String propertyName,
		...
		if (returnType == Boolean.class) {
			...
			value = (StringUtils.isNotBlank(temp) && Long.valueOf(temp) > 0) ? true
					: false;
		} else if (returnType == Long.class) {
			value = Long.valueOf(value.toString());
		} else if (returnType == Date.class) {
			try {
				value = SimpleDateFormatFactory.getInstance(
						DateUtil.FULL_TIME_FORMAT).parse(value.toString());
			} catch (ParseException e) {
				logger.error("类型转型Timpestap-->Date时，发生错误! " + e.getMessage()
						+ "(" + value.toString() + ")");
			}
