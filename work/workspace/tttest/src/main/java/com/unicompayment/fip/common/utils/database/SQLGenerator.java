/*
 * @(#)SQLGenerator.java    V1.0.0      @2014-6-18
 *
 * Project:unpcommon
 *
 * Modify Information:
...
import com.unicompayment.fip.common.utils.principle.ReflectionUtils;
...
public class SQLGenerator<T> {
	...
	public String sql_removeByColumnName(String columnName, Object val,
		...
		sql_build.append("DELETE FROM ").append(this.tableName)
				...
				.append(SQLSymbol.obtainFirstSymbol(symbol))
				.append(handleValue(val));
	 ...
	 * */
	public String sql_removeOfBatchByColumn(List<T> list,
			Map<String, String> currentColumnFieldNames, String fieldName) {
		StringBuilder sql_build = new StringBuilder();
		sql_build.append("DELETE FROM ").append(this.tableName)
		...
		for (; i < len; i++) {
			...
			Object id = ReflectionUtils.obtainFieldValue(t,
					currentColumnFieldNames.get(fieldName));
			sql_build.append(",").append(handleValue(id));
			if (i > 0 && i % (Constant.DELETE_CRITICAL_VAL - 1) == 0) {
	 ...
	 * fieldName 用于当做条件的字段名称请用大写
	 * */
	public String sql_removeOfBatchByColumnName(String columnName,
			List<Object> vals) {
		StringBuilder sql_build = new StringBuilder();
		sql_build.append("DELETE FROM ").append(this.tableName)
	...
	public String sql_modify(T t, Map<String, String> currentColumnFieldNames,
		...
		sql_build.append("UPDATE ").append(tableName).append(" SET ")
				...
				.append(pkName).append(" = ").append(handleValue(id));
	...
	private List<String> obtainColumnVals(T t,
		...
		Boolean ignore = true;
		if (null != ignoreNull && ignoreNull.length > 0) {
			ignore = ignoreNull[0];
		}
	...
	public <PK> String sql_findOneById(PK id) {
		...
		sql_build
				...
				.append(" WHERE ROWNUM = 1 AND " + pkName + " = "
						+ handleValue(id));
	...
	public String findOneByColumnName(String columnName, Object val,
		...
		sql_build
				...
				.append(" WHERE " + columnName
						+ SQLSymbol.obtainFirstSymbol(symbol)
						+ handleValue(val)).append(" AND ROWNUM = 1 ");
	 ...
	 * @return
	 */
	public <PK> String sql_findListByColumnName(String columnName, Object val,
			String... symbol) {
		StringBuilder sql_build = new StringBuilder();
		sql_build
				.append("SELECT ")
				...
				.append(" WHERE " + columnName
						+ SQLSymbol.obtainFirstSymbol(symbol)
						+ handleValue(val)).append(" ORDER BY ")
				.append(StringUtils.isEmpty(pkName) ? columnName : pkName);
	...
	public String sql_findAll() {
		...
		if (StringUtils.isEmpty(pkName)) {
			sql_build.append(" ORDER BY ").append(pkName);
		}
	...
	public <PK> String sql_isExists(String columnName, Object val,
			String... symbol) {
		StringBuilder sql_build = new StringBuilder();
		sql_build
				.append("SELECT 1 ")
